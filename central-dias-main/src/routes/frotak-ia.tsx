...
type LiveVoiceSession = {
  sendRealtimeInput: (input: { media?: Blob; audioStreamEnd?: boolean }) => void;
...
function float32ToPcm16Blob(
  input: Float32Array,
  inputSampleRate: number,
  outputSampleRate = 16000,
) {
  const ratio = Math.max(inputSampleRate / outputSampleRate, 1);
...
function FrotakIaPage() {
  ...
  const appendVoiceAssistantText = (text: string) => {
    ...
    const previousText = voiceAssistantTextRef.current;
    const separator =
      previousText && !previousText.endsWith(" ") && !/^[,.;:!?]/.test(cleanText) ? " " : "";
    voiceAssistantTextRef.current = `${previousText}${separator}${cleanText}`;
  ...
  const stopNativeAudioPlayback = () => {
    audioPlaybackGenerationRef.current += 1;
    outputSourcesRef.current.forEach((source) => {
      try {
        source.stop();
      } catch {
        // O source pode ja ter encerrado.
      }
      source.disconnect();
    });
    outputSourcesRef.current = [];
    nextAudioStartRef.current = outputAudioContextRef.current?.currentTime ?? 0;
    assistantSpeakingRef.current = false;
  };
  ...
  const playVoiceChunk = async (data: string, mimeType?: string) => {
    ...
    source.onended = () => {
      ...
      if (outputSourcesRef.current.length === 0) assistantSpeakingRef.current = false;
    };
    assistantSpeakingRef.current = true;
      ...
      content.modelTurn.parts.forEach((part) => {
        ...
        if (part.inlineData?.data) {
          ...
          playbackQueueRef.current = playbackQueueRef.current
            .then(() => {
              if (audioPlaybackGenerationRef.current !== playbackGeneration) return;
              return playVoiceChunk(part.inlineData!.data!, part.inlineData?.mimeType);
            })
            .catch((error) => {
  ...
  const startVoice = async () => {
    ...
    try {
      ...
      const voiceToken = await createFrotakAiVoiceToken({
      ...
      sessionRef.current = session as LiveVoiceSession;
      ...
      processor.onaudioprocess = (event) => {
        ...
        if (!liveSession || stoppingVoiceRef.current) return;

        const input = event.inputBuffer.getChannelData(0);
        const level = rms(input);
        if (assistantSpeakingRef.current && level > SPEECH_RMS_THRESHOLD) {
          stopNativeAudioPlayback();
          setVoiceState("listening");
        }

        liveSession.sendRealtimeInput({
          media: float32ToPcm16Blob(input, audioContext.sampleRate),
        });
      };
    } catch (error) {
